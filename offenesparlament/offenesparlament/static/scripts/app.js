...
},{"./style":3,"classnames":4,"react":"react"}],3:[function(require,module,exports){
'use strict';
...
$(document).ready(function() {
  var ReactTooltip, _convert_serverfacets_to_text, _convert_text_to_serverfacet, _current_facets, _human_readable_facets, _server_facets, _update_server_facets, model, tooltip_container, visualSearch;
  tooltip_container = document.getElementById('react_tooltip_container');
  if (tooltip_container) {
    ReactTooltip = require("react-tooltip");
    React.render(React.createElement(ReactTooltip, {
      type: 'light',
      place: 'bottom',
      effect: 'solid',
      multiline: false,
  ...
  _update_server_facets = function(facets_obj) {
    _server_facets = _.pick(facets_obj, function(x, key) {
      return x.length > 1 && _.findIndex(visualSearch.searchQuery.models, function(x) {
        return _convert_text_to_serverfacet(x.get('category')) === key;
      }) < 0;
    });
    return console.log(_server_facets, visualSearch.searchQuery);
  };
  _convert_serverfacets_to_text = function(facet_list) {
    return _.map(facet_list, function(facet) {
  ...
  _convert_text_to_serverfacet = function(human_facet) {
    var server_facet;
    server_facet = _.findKey(_human_readable_facets, function(x) {
      return x === human_facet;
    });
    if (server_facet) {
      return server_facet;
    } else {
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.result != null) {
        return _update_server_facets(response.facets.fields);
      }
    }
  });
  visualSearch = VS.init({
    ...
    callbacks: {
      ...
      facetMatches: function(callback) {
        return callback(_convert_serverfacets_to_text(_.keys(_server_facets)));
      },
      valueMatches: function(human_facet, searchTerm, callback) {
        var facet;
        facet = _convert_text_to_serverfacet(human_facet);
        if (_.has(_server_facets, facet)) {
          return callback(_.map(_server_facets[facet], function(x) {
            return x[0];
          }));
        }
      }
  ...
  visualSearch.searchQuery.on('add change', function(facet) {
    if ((facet != null ? facet.get('category') : void 0) === _human_readable_facets['llps']) {
      return console.log('change to permanent');
    }
  });
  visualSearch.searchQuery.on('all', function(which, facet) {
  ...
  return visualSearch.searchQuery.add(model, {
    at: 0
  });
});


},{"./components/SearchResults.cjsx":161,"react":"react","react-tooltip":2,"underscore":159}],161:[function(require,module,exports){
var Main, React;
...
Main = React.createClass({displayName: "Main",
  ...
  render: function() {
    var results;
    results = this.props.results.map((function(_this) {
      return function(r) {
        if (r.title != null) {
          return React.createElement("li", null, r.title);
        } else if (r.full_name != null) {
          return React.createElement("li", null, r.full_name);
        }
      };
    })(this));
    return React.createElement("ul", null, results);
  }
});
